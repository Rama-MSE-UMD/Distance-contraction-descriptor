...
def load_reference_data():
    ...
    global bond_mean_stddev_mg, bond_mean_stddev_ca, bond_mean_stddev_al, bond_mean_stddev_zn
    ...
    bond_mean_stddev_zn = pd.read_csv("Zn_bond_stddev.csv")
...
# Ordered Li first, then the other supported mobile ions — this is also the
# preset default sequence used for MOBILE_ION in the notebook (Section 2).
OXI_STATE = {"Li": 1, "Na": 1, "Mg": 2, "K": 1, "Ca": 2, "Al": 3, "Zn": 2}
...
def path_bond_distances_cutoff(structure1, structure2, site_index1,
                               site_index2, element_symbol, bond_std,
                               na_cat_dist,
    ...
    neighbor_data = defaultdict(lambda: [None])
    mean_lookup = bond_std.set_index("Element")["Mean_Distance"]
    for si, st in [(site_index1, structure1), (site_index2, structure2)]:
        st = st.copy()
        target = st[si]
        all_elements = sorted(set(sp.symbol for sp in st.composition.elements))
        for el in all_elements:
            neighbor_data.setdefault((target.specie.symbol, el), [None])
        for nbr in st.get_sites_in_sphere(target.coords, cutoff):
            key = (target.specie.symbol, nbr[0].specie.symbol)
            neighbor_data[key] = [round(nbr[1], 3)] if neighbor_data[key] == [None] else neighbor_data[key] + [round(nbr[1], 3)]

    distance_stats = {}
    for pair, distances in neighbor_data.items():
        m = mean_lookup.get(pair[1])
        min_dist = min(distances) or m or na_cat_dist[pair[1]]
        filtered = [d for d in distances if d and d <= min_dist + 0.7] or [min_dist]
        distance_stats[pair] = {
...
def run_pipeline(element_symbol, element_oxi, st_name,
    ...
    if create_folder == "Yes":
        ...
        os.chdir(folder)
        print(f"Output folder: {os.path.abspath(os.getcwd())}")
    ...
    if create_folder == "Yes":
        ...
        print(f"  → Wrote {folder}/{st_name}_linear_neb_paths.cif "
              f"(all candidate pairs, linear geometry)")
    ...
    for p_index, p in enumerate(unique_symm_paths):
        ...
        _path_files_written = 0   # counts CIFs written for this path, for the summary print below
        ...
        if create_folder == "Yes":
            ...
            os.chdir("../")
            _path_files_written += len(neb_structures) + 1
            print(f"  → Wrote {_path_files_written} CIF file(s) for path {p_index + 1} "
                  f"to '{folder}/neb_{p_index + 1}*.cif' and '{folder}/{img_dir}/'")
    ...
    _data_csv_loc = (f"{folder}/{st_name}_data.csv" if create_folder == "Yes"
                     else f"{st_name}_data.csv")
    print(f"\n  → Wrote {_data_csv_loc}  "
          f"({len(low_energy_df)} rows — all candidate pairs, symmetry-mapped)")
        ...
        for neb_idx in sorted_df["NEB1"]:
            ...
            if dim not in rel_dists:
                ...
                if dim == 1 and create_folder == "Yes":
                    ...
                    print(f"  → Wrote {folder}/1Dim_struct_{label}.cif "
                          f"(1-D percolation network, contraction = {dev:.3f} Å)")
                elif dim == 2 and create_folder == "Yes":
                    Structure.from_sites(
                        baseref.sites + rel_struct.sites
                    ).to(f"2Dim_struct_{label}.cif", "cif")
                    print(f"  → Wrote {folder}/2Dim_struct_{label}.cif "
                          f"(2-D percolation network, contraction = {dev:.3f} Å)")
                elif dim == 3:
                    if create_folder == "Yes":
                        Structure.from_sites(
                            baseref.sites + rel_struct.sites
                        ).to(f"3Dim_struct_{label}.cif", "cif")
                        print(f"  → Wrote {folder}/3Dim_struct_{label}.cif "
                              f"(3-D percolation network, contraction = {dev:.3f} Å)")
                    break

        # Dimensionality can jump directly from one level to a higher one
        # (e.g. 1-D straight to 3-D) without any path producing exactly the
        # intermediate value, leaving that key unrecorded. When that happens,
        # fill it with the next higher dimensionality that was recorded —
        # reaching dimension N implies dimension N-1 was already satisfied
        # at or before that same contraction value, so the higher value is
        # a valid (if conservative) estimate for the missing one.
        for missing_dim in (2, 1, 0):
            if missing_dim not in rel_dists:
                for higher_dim in range(missing_dim + 1, 4):
                    if higher_dim in rel_dists:
                        rel_dists[missing_dim] = rel_dists[higher_dim]
                        break
        return rel_dists
    ...
    if create_folder == "Yes":
        ...
        print(f"  → Wrote {folder}/{st_name}_dev.csv  (dimensionality summary)")
        print(f"  → Wrote {folder}/{st_name}_uniquepair.csv  (all pairs, symmetry-mapped)")
        print(f"  → Wrote {folder}/{st_name}_uniquesymm.csv  (symmetry-unique paths only)")
        out_loc = os.getcwd()   # already inside `folder` at this point
        os.chdir("..")
    else:
        df_out.to_csv(f"{st_name}_deviation.csv", index=False)
        print(f"  → Wrote {st_name}_deviation.csv  (dimensionality summary)")
        out_loc = os.getcwd()

    print(f"\nDone. All output files for '{st_name}' are in: {out_loc}")
