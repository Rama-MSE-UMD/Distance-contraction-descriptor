# -*- coding: utf-8 -*-
"""
distance_contraction/pipeline.py

Server-ready version of the "Distance Contraction — Ion-Migration Pathway
Screener" (originally a Colab notebook by Ramanuja Srinivasan Saravanan,
University of Maryland, supervised by Prof. Yifei Mo).

This module strips out all Colab/ipywidgets-specific code and exposes:

    setup_reference_data(cache_dir)   -> downloads/loads shared reference
                                          data (periodic table + bond-distance
                                          stats) ONCE, from GitHub raw URLs.
    run_pipeline(...)                 -> the exact same analysis pipeline as
                                          the notebook, unchanged internally.

The calling web app (app.py) is responsible for:
  - Running each request inside its own working directory (so concurrent
    users' output files don't collide).
  - Saving the user's uploaded CIF file into that directory before calling
    run_pipeline().
  - Serializing calls to run_pipeline() with a lock, since this module uses
    os.chdir() internally (process-wide state) exactly as the original
    notebook did.
"""
...
_BASE_URL = "https://raw.githubusercontent.com/Rama-MSE-UMD/Distance-contraction-descriptor/main/"
...
# Populated by setup_reference_data(); referenced as module globals by the
...
def setup_reference_data(cache_dir):
    """Download (if needed) and load the shared reference datasets.

    Downloads go to *cache_dir* and are skipped on subsequent calls if the
    files already exist there — safe to call once at app startup.
    """
    ...
    for fname in _REFERENCE_FILES:
        ...
        if os.path.exists(fpath):
            continue
        urllib.request.urlretrieve(_BASE_URL + fname, fpath)

    bond_mean_stddev_na = pd.read_csv(os.path.join(cache_dir, "Na_bond_stddev.csv"))
    bond_mean_stddev_li = pd.read_csv(os.path.join(cache_dir, "Li_bond_stddev.csv"))
    bond_mean_stddev_k  = pd.read_csv(os.path.join(cache_dir, "K_bond_stddev.csv"))
    bond_mean_stddev_mg = pd.read_csv(os.path.join(cache_dir, "Mg_bond_stddev.csv"))
    bond_mean_stddev_ca = pd.read_csv(os.path.join(cache_dir, "Ca_bond_stddev.csv"))
    bond_mean_stddev_al = pd.read_csv(os.path.join(cache_dir, "Al_bond_stddev.csv"))
    # bond_mean_stddev_zn = pd.read_csv(os.path.join(cache_dir, "Zn_bond_stddev.csv"))

    with open(os.path.join(cache_dir, "periodic_table.json")) as f:
        _ptd = json.load(f)
    Periodic_table_data = pd.DataFrame(_ptd).transpose()

    most_common_oxidation_states = {
        el.symbol: (el.common_oxidation_states[0]
                    if el.common_oxidation_states else None)
        for el in Element
    }

    _OXI_STATE = {"Na": 1, "Li": 1, "K": 1, "Mg": 2, "Ca": 2, "Al": 3}
...
def oxidation_state_for(element_symbol):
    """Public helper so app.py can validate the element before running."""
    if _OXI_STATE is None:
        raise RuntimeError("setup_reference_data() must be called first.")
    return _OXI_STATE.get(element_symbol)


def supported_elements():
    return ["Na", "Li", "Mg", "K", "Ca", "Al", "Zn"]


# ════════════════════════════════════════════════════════════════════════════
# Section 3 — Helper functions (verbatim from the notebook)
# ════════════════════════════════════════════════════════════════════════════
# ════════════════════════════════════════════════════════════════════════════
...
print("Structure utility functions defined.")
...
print("Symmetry-reduction function defined.")
...
print("Path-generation and connectivity functions defined.")

# ════════════════════════════════════════════════════════════════════════════
# 3.4  Bond-distance statistics along a migration path
# ════════════════════════════════════════════════════════════════════════════

def path_bond_distances_cutoff(structure1, structure2, site_index1,
                               site_index2, element_symbol,bond_std,na_cat_dist,
    ...
    for si, st in [(site_index1, structure1), (site_index2, structure2)]:
        st     = st.copy()
        target = st[si]
        for nbr in st.get_sites_in_sphere(target.coords, cutoff):
            neighbor_data[(target.specie.symbol, nbr[0].specie.symbol)].append(round(nbr[1],3))
...
print("Bond-distance statistic functions defined.")
...
print("Transverse-optimisation functions defined.")


# ════════════════════════════════════════════════════════════════════════════
# Main analysis pipeline (verbatim from the notebook)
# ════════════════════════════════════════════════════════════════════════════

def run_pipeline(element_symbol, element_oxi, st_name,
                 max_path_length, nimages, create_folder):
    ...
    """
    params   = _ION_PARAMS[element_symbol]
    bond_std = _BOND_STDDEV.get(element_symbol, bond_mean_stddev_na)
    ...
    for p_index, p in enumerate(unique_symm_paths):
        ...
        def _last_cation(d):
            vals = [v[-1] for k, v in d.items()
                    if k in cation_groups and v and v[-1] is not None]
            return max(vals) if vals else 0.0
        ...
        for mr, cat_l, ani_l, cat_cat, rd_list, ref_label in [
        ...
        # ── Percolation radius ────────────────────────────────────────────
        # Percolation radius = (TS bond distance) − (ionic radius of neighbour)
        # Represents the effective free space available to the mobile ion at TS.
        ts_site  = mod_sites[ts_idx]
        ts_nbrs  = struct_orig.get_sites_in_sphere(
            ts_site.coords, max(na_cat_dist.values()))
        perc_ele = {el: [] for el in list(na_cat_dist) + [anion_symbol]}
        for nbr in ts_nbrs:
            el = nbr.specie.symbol
            if el in perc_ele and ts_site.distance(nbr) > 0:
                perc_ele[el].append(
                    ts_site.distance(nbr) - weighted_ionic_radius[el])
        perc_ele.pop(element_symbol, None)
        perc_radius = min(
            (min(v) for v in perc_ele.values() if v), default=None)
        perc_radius_list.append(perc_radius)
    ...
    dim_path   = _dim_analysis("Max_contraction_path",
                               "Max_contraction_path",   "path")
    ...
    print("\nDone.")
    return df_out, low_energy_df, dim_struct, dim_path, dim_stddev, st_name, structure
